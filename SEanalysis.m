% This code reads in, calculates and organizes the statistics of the
% semi-empirical analyses developed by Klaus Bittermann for the
% publication.
%
...
% pool across each individual model
slcmip_models=nan(size(slcmip,1)*2,size(slcmip,2),size(slcmip,3),2);
slcmip_models_diff=nan(size(slcmip,1)*2,size(slcmip,2),size(slcmip,3));
for k=1:4
    
    % determine the indices we are pooling with
    tind3=1:2:2000; % Marcott or Mann samples
    sind3=NaN;      % Historical or CF samples
    switch k
    case 1
    	sind3=1;
    case 2
    ...
    case 4
        ...
        sind3=2;
    otherwise
      error('something went wrong in the switch/loop')
    end
    
    % store the the pooled samples
    slcmip_models(tind3,:,:,sind3)=slcmip(:,:,:,k);
end
% store the difference model samples
slcmip_models_diff(1:2:2000,:,:)=sldiffcmip(:,:,:,1);
slcmip_models_diff(2:2:2000,:,:)=sldiffcmip(:,:,:,2);
...
%   qmodels      --> [NaN]
%   qdiffmodels  --> [NaN]
%   slmodels     --> slcmip_models(:,:,:,history)
%   sldiffmodels --> [NaN]
                ...
                'cf', ... 
                    struct('summary', ...
                        struct('q', qslcmip_summary(:,:,2), ...
                               ...
                               'qmodels', [NaN], ...
                               'qdiffmodels', [NaN], ...
                               'slmodels', slcmip_models(:,:,:,2), ...
                               'sldiffmodels', slcmip_models_diff), ...
                     'marcott', ...
